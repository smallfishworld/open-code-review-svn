...
SVN ワーキングコピーでは、同じコマンドが `svn status --xml` と `svn diff` を使用し、変更、追加予定、削除予定、および未管理のファイルをレビューします。未管理ディレクトリは再帰的に展開されます。SVN サポートには SVN 1.9 以降が必要で、現在はワークスペースモードのみ対応しています。範囲モードと単一 commit モードは Git 専用です。
