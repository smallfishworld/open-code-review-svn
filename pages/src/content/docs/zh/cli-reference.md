...
在 SVN 工作副本中，相同命令通过 `svn status --xml` 和 `svn diff` 审查已纳入版本控制的修改、计划新增和计划删除文件。未版本控制（`?`）文件会被跳过，需要先执行 `svn add <path>` 才会进入审查。SVN 支持要求 SVN 1.9+，目前仅支持工作区模式；区间和单 commit 模式仍仅支持 Git。
