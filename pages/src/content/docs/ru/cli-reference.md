...
В рабочей копии SVN та же команда использует `svn status --xml` и `svn diff`
для проверки изменённых, запланированных к добавлению или удалению и
неотслеживаемых файлов. Неотслеживаемые каталоги обходятся рекурсивно. Требуется
SVN 1.9 или новее; поддерживается только режим рабочей области, а диапазон и
отдельный коммит по-прежнему доступны только для Git.
