...
- **Git >= 2.41** — требуется для ревью рабочей копии Git, диапазона и отдельного коммита.
- **SVN >= 1.9** — требуется только для ревью незакоммиченных изменений в рабочей копии SVN.
...
# Режим рабочей копии SVN — ревью изменённых, добавленных и удалённых файлов
# Режимы диапазона и отдельного коммита пока доступны только для Git.
...
### Ревью рабочей копии SVN

Этот форк поддерживает ревью незакоммиченных изменений в рабочей копии SVN (Apache Subversion) перед коммитом. OCR автоматически определяет корень рабочей копии, поэтому команду можно запускать из корня или любого подкаталога.

В режиме рабочей копии SVN поддерживаются:

- Изменённые, запланированные к добавлению и запланированные к удалению файлы из `svn status`.
- Версионируемые изменения из `svn diff`. Чтобы новый файл попал в ревью, сначала выполните `svn add <path>`; неотслеживаемые (`?`) файлы намеренно пропускаются.
- Текстовые и бинарные изменения, имена файлов с пробелами, а также те же правила ревью и форматы вывода, что и для рабочей копии Git.

Проверка текущей рабочей копии SVN перед коммитом:
...
Можно также явно указать рабочую копию и сохранить результат:
...
Сейчас SVN поддерживает только ревью рабочей копии. Режимы на основе истории Git, включая `--from`, `--to`, `--commit` и возобновление сессии, недоступны для рабочих копий SVN. Файлы, игнорируемые SVN, не проверяются; чтобы включить их, выполните `svn add` или измените свойства игнорирования SVN.
